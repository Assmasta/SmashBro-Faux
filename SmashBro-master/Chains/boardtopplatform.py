...
class BoardTopPlatform(Chain):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        on_side_platform = smashbro_state.on_ground and smashbro_state.position.y > 5
        above_top_platform = (not smashbro_state.on_ground) and (smashbro_state.position.y + smashbro_state.ecb.bottom.y > platform_height) and \
            plat_position[1] < smashbro_state.position.x < plat_position[2]

        if smashbro_state.on_ground and smashbro_state.action != Action.KNEE_BEND:
            self.interruptible = True
        ...
        if smashbro_state.action in [Action.JUMPING_FORWARD, Action.JUMPING_BACKWARD]:
            if gamestate.stage == Stage.BATTLEFIELD:
                if smashbro_state.action_frame == 14:
            ...
            if gamestate.stage == Stage.DREAMLAND:
                if smashbro_state.action_frame == 16:
                    self.controller.press_button(melee.Button.BUTTON_Y)
                    self.controller.tilt_analog(melee.Button.BUTTON_MAIN, 0.5, 0.5)
                    self.interruptible = False
                    return
            if gamestate.stage == Stage.YOSHIS_STORY:
                if smashbro_state.action_frame == 21:
                    self.controller.press_button(melee.Button.BUTTON_Y)
                    self.controller.tilt_analog(melee.Button.BUTTON_MAIN, 0.5, 0.5)
                    self.interruptible = False
                    return
        ...
        # Drift into opponent
        if smashbro_state.action in [Action.JUMPING_ARIAL_FORWARD, Action.JUMPING_ARIAL_BACKWARD, Action.JUMPING_FORWARD, Action.JUMPING_BACKWARD]:
            self.interruptible = False
            self.controller.release_button(melee.Button.BUTTON_Y)
            self.controller.tilt_analog(melee.Button.BUTTON_MAIN, int(smashbro_state.position.x < opponent_state.position.x), 0.5)
            return

        # Dash at the position of the opponent
        if smashbro_state.on_ground and smashbro_state.position.y < 10:
            self.interruptible = True
            ...
            if abs(smashbro_state.position.x - pivotpoint) < 5 and smashbro_state.action == Action.TURNING:
                self.interruptible = False
                self.controller.press_button(melee.Button.BUTTON_Y)
                return
            ...
            if smashbro_state.action == Action.DASHING and smashbro_state.action_frame >= 11:
                ...
                return
            self.controller.tilt_analog(melee.Button.BUTTON_MAIN, int(smashbro_state.position.x < pivotpoint), 0.5)
            return
