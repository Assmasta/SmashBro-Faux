...
class Dropdownshine(Chain):
    ...
    def inrange(smashbro_state, opponent_state, framedata):
        ...
        frames_y = abs(opponent_state.position.y - smashbro_state.position.y) // 3.4
        ...
        frames_x = abs(opponent_state.position.x - smashbro_state.position.x) // (0.819625854 / 2)
        ...
        latefirefox = opponent_state.character in [Character.FOX, Character.FALCO] and \
            opponent_state.action == Action.SWORD_DANCE_3_LOW and (smashbro_state.invulnerability_left < framesleft)
        ...
        if (frames_x <= frames_y) and (smashbro_state.invulnerability_left >= frames_y) and not latefirefox:
            return True
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if smashbro_state.invulnerability_left == 0 and smashbro_state.action != Action.EDGE_HANGING:
        ...
        if smashbro_state.action == Action.DOWN_B_STUN:
            self.interruptible = True
            controller.press_button(Button.BUTTON_Y)
            return

        if smashbro_state.action in [Action.EDGE_CATCHING]:
            self.interruptible = True
        ...
        if smashbro_state.action == Action.EDGE_HANGING:
            ...
            controller.tilt_analog(melee.Button.BUTTON_C, int(not smashbro_state.facing), 0.5)
            return
        ...
        elif opponent_state.position.y > smashbro_state.position.y:
            self.interruptible = True
            controller.release_all()
            return
        ...
        if smashbro_state.action == Action.FALLING and smashbro_state.speed_y_self > -3.35:
            self.interruptible = False
            controller.tilt_analog(melee.Button.BUTTON_MAIN, 0.5, 0)
            return
