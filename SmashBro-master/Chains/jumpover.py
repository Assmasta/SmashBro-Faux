...
class JumpOver(Chain):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if smashbro_state.action in [Action.WALK_SLOW, Action.WALK_MIDDLE, Action.WALK_FAST]:
            self.interruptible = True
            controller.release_all()
            return

        landing_direction = smashbro_state.position.x < self.landing_spot
        ...
        if smashbro_state.action in [Action.DASHING, Action.RUNNING]:
            if abs(smashbro_state.position.x - self.landing_spot) < starting_distance:
                if smashbro_state.facing == landing_direction:
