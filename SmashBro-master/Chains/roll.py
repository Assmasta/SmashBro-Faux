...
class Roll(Chain):
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if smashbro_state.action in [Action.SHIELD_REFLECT, Action.SHIELD, Action.SHIELD_START]:
            if controller.prev.main_stick != (.5, .5):
                controller.tilt_analog(Button.BUTTON_MAIN, .5, .5)
                return
            self.interruptible = False
            controller.tilt_analog(Button.BUTTON_MAIN, int(smashbro_state.position.x < opponent_state.position.x), 0.5)
            return

        # Let go once we're in the roll
        if smashbro_state.action in [Action.ROLL_BACKWARD, Action.ROLL_FORWARD]:
            self.interruptible = True
            controller.empty_input()
            return
