...
class Defend(Tactic):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        # grab_response_min = random.randint(12, 30)
        # grab_response_max = random.randint(grab_response_min, 30)
        # if not dangerous_up_b and is_grab and (opponent_state.character != Character.SAMUS or grab_response_min <= framesuntilhit <= grab_response_max):
        ...
        if shield_response_min < framesuntilhit <= shield_response_max and not is_grab:
            if 1 <= framesuntilhit and opponent_state.action in [Action.GROUND_ATTACK_UP, Action.GETUP_ATTACK] \
                    and smashbro_state.percent <= 90 and smashbro_state.action in (Action.STANDING, Action.RUNNING):
        ...
        if smashbro_state.action in [Action.SHIELD, Action.SHIELD_START, Action.SHIELD_STUN, Action.SHIELD_REFLECT]:
            ...
            if hold == False:
                ...
                # tracker adjustment
                if gamestate.custom["aerial_fraction"] > 50:
