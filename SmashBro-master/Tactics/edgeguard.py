...
class Edgeguard(Tactic):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if Dropdownshine.inrange(smashbro_state, opponent_state, self.framedata):
        ...
        if smashbro_state.action in [Action.EDGE_HANGING, Action.EDGE_CATCHING]:
            ...
            # if they're about to land on stage, bair them off
            if opponent_state.action in [Action.DEAD_FALL]:
                edgedistance = melee.stages.EDGE_GROUND_POSITION[gamestate.stage] - abs(opponent_state.position.x)
                if abs(edgedistance) < 15 and edgedistance > 0 and 0 < opponent_state.position.y < 15:
                    print("edge late bair conditions met")
                    # self.pickchain(Chains.Dedgebair)
                return
            ...
            elif smashbro_state.action == Action.EDGE_HANGING and smashbro_state.action_frame == 1:
            ...
            if gamestate.distance < 11.8 and opponent_state.character in [Character.FOX, Character.FALCO, Character.JIGGLYPUFF] and \
                    smashbro_state.invulnerability_left <= 1:
        ...
        # We are on the stage
            ...
            if (not recoverhigh or randomgrab) and not onedge and opponent_state.invulnerability_left < 5 and edgedistance < 10 and smashbro_state.on_ground:
