...
class Infinite(Tactic):
    ...
    def caninfinite(smashbro_state, opponent_state, gamestate, framedata, difficulty):
        isroll = framedata.is_roll(opponent_state.character, opponent_state.action)

        if gamestate.custom["shine_count"] > 1:
            return False
        ...
        if smashbro_state.position.y > 2 or opponent_state.position.y > 2:
            return False
        ...
        if (smashbro_state.action == Action.DASHING and smashbro_state.action_frame >= 11):
            if (smashbro_state.speed_ground_x_self > 0) == (smashbro_state.position.x > 0):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        distance_from_edge = melee.stages.EDGE_GROUND_POSITION[gamestate.stage] - abs(smashbro_state.position.x)
        edgetooclose = smashbro_state.action == Action.EDGE_TEETERING_START or distance_from_edge < 5
        ...
        if gamestate.distance < shinerange and not edgetooclose:
            ...
            if smashbro_state.action in [Action.RUNNING, Action.RUN_BRAKE, Action.CROUCH_START] and distance_from_edge < 16:
                self.chain = None
                self.pickchain(Chains.Waveshine)
                return
            ...
            if (smashbro_state.position.x < opponent_state.position.x < 0) or (0 < opponent_state.position.x < smashbro_state.position.x):
                self.chain = None
                self.pickchain(Chains.Waveshine)
                return
            ...
            if (smashbro_state.speed_ground_x_self > 0) == onright:
                self.chain = None
                self.pickchain(Chains.Waveshine)
                return
        ...
        if not (smashbro_state.action == Action.DOWN_B_GROUND_START and smashbro_state.action_frame in [1,2]):
