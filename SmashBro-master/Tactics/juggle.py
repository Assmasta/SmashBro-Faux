...
class Juggle(Tactic):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if not on_ground:
            ...
            in_range = (abs(end_early_x - smashbro_state.position.x) < 8) and (abs(end_early_y - smashbro_state.position.y) < 12)
            if smashbro_state.action in [Action.TURNING, Action.STANDING] and in_range and (7 <= frames_left):
                if facing_away or abs(end_early_y - smashbro_state.position.y) > 5:
                    self.chain = None
                    # shffl to mix up light fast-fallers, end juggle
                    if light and fastfaller and opponent_state.percent > 89:
                        if facing_away:
                        ...
                        if smashbro_state.action in [Action.TURNING]:
                ...
                elif smashbro_state.position.y + 2 < end_early_y:
                    self.chain = None
            ...
            for height_level in AirAttack.height_levels():
                ...
                if (commitment-3 < frames_left) and (abs(smashbro_state.position.x - end_early_x) < 20):
                    # It's okay if SmashBot's end position is no more than 5 units above the target, and 10 below
                    if -10 < ((height+smashbro_state.position.y) - end_early_y) < 5:
                        if self.logger:
                        ...
                        self.chain = None
                        # if near edge, 50% chance of bair
                        juggle_rng = random.randint(0, 1)
                        if juggle_rng == 0 and melee.stages.EDGE_GROUND_POSITION[gamestate.stage] - abs(smashbro_state.position.x) < 30:
                            self.pickchain(Chains.AirAttack, [end_early_x, end_early_y, height_level, AIR_ATTACK_DIRECTION.BACK])
                            if smashbro_state.action_frame == 1:
                                print('juggle bair')
                            return
                        else:
            ...
            if self.framedata.is_roll(opponent_state.character, opponent_state.action):
                ...
                if frames_left > 10:
                    ...
                    if side_plat_height is not None and (frames_left > 25) and abs(side_plat_height - opponent_state.position.y) < 5:
                        ...
                        smashbro_on_side_plat = smashbro_state.on_ground and abs(smashbro_state.position.y - side_plat_height) < 5
                        if side_plat_left < smashbro_state.position.x < side_plat_right and not smashbro_on_side_plat:
                            self.chain = None
                ...
                if (abs(smashbro_state.position.x - end_x) < 5) and (abs(smashbro_state.position.y - end_y) < 20):
                    # Pivot
                    ...
                    if smashbro_state.action in [Action.TURNING, Action.STANDING]:
                                ...
                                if light and fastfaller and opponent_state.percent > 89:
                                    if facing_away:
                                        ...
                                        if smashbro_state.action_frame == 1:
                                            print('juggle nair')
                                        return
                                elif light and fastfaller and opponent_state.percent < 90:
                                    if facing_away and smashbro_state.action == Action.STANDING:
                                        self.pickchain(Chains.Tilt, [TILT_DIRECTION.UP])
                                        if smashbro_state.action_frame == 1:
                                            print('juggle utilt')
                                        return
                        ...
                        if frames_left == 1 and gamestate.distance < 10 and not tech_miss_on_plat:
                            ...
                            if smashbro_state.action in [Action.DOWN_B_GROUND] and smashbro_state.action_frame == 1:
                                print('platform-boarding waveshine')
                            return
                        else:
                ...
                elif (abs(smashbro_state.position.x - end_x) < 10):
