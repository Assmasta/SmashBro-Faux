...
class Retreat(Tactic):
    ...
    def step(self, gamestate, smashbro_state, opponent_state):
        ...
        if oos:
            if gamestate.distance < 14.5 and opponent_state.invulnerability_left < 8 and facingop:
        ...
        if abs(opponent_state.position.y - smashbro_state.position.y) < 5 and not missle_approaching:
        ...
        if (side_plat_height is not None) and (opponent_state.position.y+1 > side_plat_height) and (side_plat_left < opponent_state.position.x < side_plat_right):
            ...
            if smashbro_state.position.y > 5 and smashbro_state.on_ground:
                ...
                if (other_side_plat_left+5 < smashbro_state.position.x < other_side_plat_right-5) and not missle_approaching:
                    self.pickchain(Chains.Laser)
                    return
                else:
                    self.chain = None
